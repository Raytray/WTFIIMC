...
public class ListEventsActivity extends Activity {
	...
	private class GetAllEvents extends AsyncTask<String, Integer, String> {
		...
		protected void onPostExecute(String result) {
			...
			/*
			 * final ArrayList<String> list = new ArrayList<String>(); try{
			 * JSONObject jObject = new JSONObject(result); JSONArray
			 * resultsArray = jObject.getJSONArray("results"); for(int
			 * i=resultsArray.length()-1; i>= 0; i--){ JSONObject eventObject =
			 * resultsArray.getJSONObject(i);
			 * 
			 * list.add(eventObject.getString("name")); } final
			 * StableArrayAdapter adapter = new
			 * StableArrayAdapter(getApplicationContext(),
			 * android.R.layout.simple_list_item_1, list);
			 * listEvents.setAdapter(adapter); } catch(Exception e){
			...
			try {
				...
				ListAdapter adapter = new SimpleCursorAdapter(
						getApplicationContext(), R.layout.list_item, cursor,
						col, to);
