...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		submitButton.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				if (isEmpty(nameField) || isEmpty(eventField)
						|| isEmpty(descField) || isEmpty(startDate)
						|| isEmpty(endDate)) {
					returnResults.setText("Missing required fields");
				} else {
					// returnResults.setText(nameField.getText().toString() +
					// " created the event: " +
					// eventField.getText().toString());

					new PostNewEvent().execute(eventField.getText().toString(),
							descField.getText().toString(), startDate.getText()
	...
	private class PostNewEvent extends AsyncTask<String, Integer, String> {
		...
		protected String doInBackground(String... param) {
			...
			try {
				...
				nameValuePairs.add(new BasicNameValuePair("event_info",
						param[1]));
		...
		protected String doInBackground(String... param) {
			...
			try {
				...
				for (int i = resultsArray.length() - 1; i >= limitFive; i--) {
					...
					jsonToString.append(eventObject.getString("id") + " "
